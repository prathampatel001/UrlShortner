    ...
    const urlClickData = await Url.aggregate([
      {
        $lookup: {
          ...
          localField: '_id', // The field in the Url collection
          foreignField: 'url_id', // The field in the Session collection
      ...
      },
      {
        $project: {
          originalUrl: 1,
          shortUrl: 1,
          clicks: { $size: '$sessionData' },
        },
      },
    ]);

    res.status(200).json({
      message: 'All URLs and their click counts fetched successfully',
      data: urlClickData,
    });
  } catch (error) {
